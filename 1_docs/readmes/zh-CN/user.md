...
完整版本号由两组语义化版本号拼接而成，格式为：`Bx.y.zCx.y.z`  
（例如：`B0.2.0C0.27.0`）

- **B 版本号**：反映网络 API 版本，遵循常规语义化版本定义（Major.Minor.Patch）。
- **C 版本号**：反映程序功能版本，按如下自定义规则定义：

  - `X`：决定性更新，范式变化、具有里程碑意义（较大可能引发部分资源文件不兼容）
  - `Y`：阶段性更新，新工具加入或旧功能改变（不包括旧功能完善）（可能引发部分资源文件不兼容）
  - `Z`：维护性更新，Bug修复、优化、适配性更新、旧功能完善（几乎不可能发生任何不兼容）

> **简单理解**：B 号变了 → API / 开发接口可能有变化；C 号变了 → 你用的软件功能有更新。
