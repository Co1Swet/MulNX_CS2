...
void MulNX::KeyCheckPack::Refresh() {
    this->Usable = (this->vkCode != 0) && (this->ComboClick != 0);
}

std::pair<std::optional<MulNX::KeyCheckPack>, bool> MulNX::KeyCheckPack::DebugWindow(const char* windowName, std::atomic<bool>& openWindow)const {
    auto w = MulNX::UI::RAIIWindow(windowName, openWindow);
    if (!w || !w.ShouldDraw())return { std::nullopt,false };
    ...
    // 修饰键复选框
    change |= ImGui::Checkbox("Ctrl", &temp.Ctrl);
    ImGui::SameLine();
    change |= ImGui::Checkbox("Shift", &temp.Shift);
    ImGui::SameLine();
    change |= ImGui::Checkbox("Alt", &temp.Alt);
    ...
    if (ImGui::Combo("##KeyCombo", &currentIndex, keyNames.data(), static_cast<int>(keyNames.size()))) {
        temp.vkCode = keyList[currentIndex].first;
        change = true;
    }
    ...
    if (ImGui::InputInt("##ComboClick", &combo, 1, 5)) {
        ...
        temp.ComboClick = static_cast<uint8_t>(combo);
        change = true;
    }
    temp.Refresh();
    if (!temp.Usable) {
        ImGui::Text("当前无效，无法提交");
    }
    ImGui::BeginDisabled(!temp.Usable);
    bool clickedConfirm = ImGui::Button("确认修改");
    ImGui::EndDisabled();
    if (clickedConfirm) {
